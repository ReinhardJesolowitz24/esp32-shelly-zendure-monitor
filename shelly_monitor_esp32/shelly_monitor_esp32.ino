 ...
 * Board  : ELECROW 7" HMI ESP32 Display, 800x480 RGB TFT (ESP32-S3-WROOM-1)
 *          (same hardware as "CrowPanel ESP32 HMI 7.0"; panel driver EK9716BD3)
 *
 * Budget ESP32-S3 port of the Arduino GIGA R1 version:
...
// --- Core-Schutz: Waechter laufen auf 2.0.x. Falscher Core (3.x) = Compile-Fehler. ---
#if defined(ESP_ARDUINO_VERSION_MAJOR) && ESP_ARDUINO_VERSION_MAJOR >= 3
#error "Falscher Core! Waechter brauchen 'esp32 by Espressif' 2.0.17 (Tools -> Board). Regler/Broker = 3.x, hier NICHT 3.x!"
#endif
...
#define FW_VERSION "esp32-2.14"  // 2.14: CONTROL_IO_MS 1000->1750 (Poll-Timeout): 750ms beob. Regler-Kalt-Poll-Max + 1000ms Reserve -> eliminiert falsche REGLER/CONTROL DOWN durch Netz-/Kalt-Connect-Jitter (Signal-Reinheit: Zaehler = echtes Problem). Basis 2.13 (kumulative Zustands-Zaehler). // in /status (Feld "fw"); "build" = Compile-Zeit
#define SCREEN_W   800
#define SCREEN_H   480
...
// Zeit + Tagessaldo
String curTime = "--:--";
String myIp = "0.0.0.0";            // eigene IP (fuer Anzeige)
...
const unsigned long CONTROL_IO_MS = 1750;      // Connect-/Read-Timeout je controlAlive. 1750 = 750ms beob. Regler-Kalt-Poll-Max + 1000ms Reserve -> eliminiert FALSCHE REGLER/CONTROL DOWN durch Netz-/Kalt-Jitter (Debounce >3 = 4x in Folge). Trade-off: blockiert bei STROMLOSEM Ziel den Anzeige-Loop bis 1750ms/Poll (2026-07-24; war 1000, davor 2500)
...
int controlBrokerClients(const char* host) {
  WiFiClient client;
  if (!client.connect(host, 80, CONTROL_IO_MS)) return -1;
  client.print("GET /status HTTP/1.0\r\nHost: monitor\r\nConnection: close\r\n\r\n");
  unsigned long t = millis();
...
#if CONTROL_WATCH_ENABLE
  c.print(",\"ctrl_state\":");    c.print(ctrlState);     // -1=unbek,0=ok,1=Regler,2=Broker,3=beide,4=MQTT down
...
void setup() {
  ...
  char hw[48]; snprintf(hw, sizeof(hw), "Flash %u MB  /  PSRAM %u MB", flashMB, psramMB);
  printAt(20, 200, "ESP32-S3 Monitor boot", 3, COL_TITLE);
  printAt(20, 250, hw, 2, (psramMB >= 2) ? COL_EINSPEIS : COL_BEZUG);  // PSRAM fehlt? -> rot
  delay(1500);
...
void loop() {
  if (wdtActive) esp_task_wdt_reset();     // Watchdog fuettern (jeden Durchlauf)
  ...
  // Eigener Timer, unabhaengig von der Shelly/Zendure-Sequenz; blockiert dank kurzer
  // Timeouts nur wenige Sekunden, falls ein Ziel tot ist.
